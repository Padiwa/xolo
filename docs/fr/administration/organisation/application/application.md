...
- **Jeton d'utilisateur multi-organisations** : `GET /api/v1/models` ne renvoie que les modèles activés de l'organisation du jeton (les autres appartenances de l'utilisateur sont ignorées, comme pour le proxy). Les modèles virtuels personnels (`~/...`) visibles suivent la même règle : ils sont filtrés aux seules organisations dans lesquelles l'utilisateur dispose de la permission `personal-vm:create` (ou du rôle owner) parmi celles retenues par le scope ci-dessus. Un utilisateur qui possède des modèles virtuels personnels dans une autre organisation que celle de son jeton ne les verra pas dans cette réponse — c'est cohérent avec le proxy, qui a toujours limité son routage à l'unique OrgID du jeton.
