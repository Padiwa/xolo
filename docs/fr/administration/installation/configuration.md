...
Sur le chemin statique (`DISCOVERY_URL` vide), `PROFILE_URL` est également **obligatoire** : sans lui, aucun `userinfo_endpoint` n'est disponible en repli et la première connexion échouerait avec une erreur goth opaque. Le serveur refuse alors de démarrer avec le message `gitea provider requires PROFILE_URL or a discovery document that publishes userinfo_endpoint`, aligné sur le comportement de la branche découverte.
