...
| `prompt-guard`      | PRE_REQUEST, POST_RESPONSE, TOOL_RESULT_INSPECTOR | Prompt injection risk without an LLM call (rules, structural signals, embedded model), pressure accumulated over the turns of a conversation, inspection of tool results and of the answer with planted canaries, optional block and redaction |
