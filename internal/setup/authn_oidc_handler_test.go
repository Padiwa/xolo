...
func TestBuildGiteaProviderToleratesMissingJWKSURI(t *testing.T) {
	...
	t.Run("empty DiscoveryURL with empty ProfileURL fails boot", func(t *testing.T) {
		// Symmetric with the discovery-populated branch which already
		// refuses to boot when neither PROFILE_URL nor userinfo_endpoint
		// is set. On the static-config path there is no userinfo_endpoint
		// to fall back to: gitea.NewCustomisedURL accepts an empty
		// profileURL, but goth then builds the profile request as
		// profileURL + "?access_token=..." which surfaces at first login
		// as an opaque net/http "unsupported protocol scheme \"\"" error.
		// Refusing to boot keeps both paths consistent and surfaces an
		// actionable error at startup.
		_, _, _, err := buildGiteaProvider(
			context.Background(),
			slog.Default(),
			newOIDCDiscoveryHTTPClient(),
		...
		if err == nil {
			t.Fatal("error: got nil, want a startup refusal on the empty PROFILE_URL")
		}
		if !strings.Contains(err.Error(), "PROFILE_URL") {
			t.Errorf("error: got %q, want it to mention PROFILE_URL", err)
		}
		if !strings.Contains(err.Error(), "userinfo_endpoint") {
			t.Errorf("error: got %q, want it to mention the discovery-document fallback", err)
		}
	})
