...
func buildGiteaProvider(
	...
	if discoveryURL != "" {
		...
		//
		// On the static path an empty PROFILE_URL cannot be tolerated: the
		// discovery-populated branch falls back to userinfo_endpoint, but on
		// the static branch there is nothing to fall back to, and the failure
		// would only surface on first login as an opaque goth error against
		// a profile endpoint that never receives a request. Refuse to boot
		// with the same wording as the discovery branch to keep both paths
		// symmetric.
		if profileURL != "" {
			if err := requireAbsoluteHTTPURL(profileURL, "PROFILE_URL"); err != nil {
				return nil, oidc.Provider{}, nil, errors.WithStack(err)
		...
		logger.WarnContext(
			ctx,
			"gitea provider has no discovery url; JWT id-token and opaque-token validation are disabled and interactive login uses the static AUTH_URL / TOKEN_URL configuration",
